...
export class ResultPage extends BasePage {
    ...
    async lifeTermResultJourney(scenario: LifeTermScenario): Promise<boolean> {
        ...
        const shouldStop = await this.validateNegativeCasesScenarios(scenario.type, scenario.expected, scenario.planName, scenario.tcId);
        if (shouldStop) {
    ...
    private async validateNegativeCasesScenarios(type?: string, expected?: string, planName?: string, tcId?: string): Promise<boolean> {
        ...
        if (tcId === "TC_13") {
            const ageErrorMsg = this.page.locator("text=Cover upto age must be at least 23");
            const isAgeErrorVisible = await ageErrorMsg.isVisible({ timeout: 5000 }).catch(() => false);

            if (isAgeErrorVisible) {
                this.log("Age validation message displayed for TC_14 - stopping journey");
                await this.fullScreenScreenshot("Result Page: Age Validation Message Screenshot");
                return true;
            }
        }
        ...
        if (tcId === "TC_15") {
            ...
            if (!isCriticalIllnessCoverVisible) {
                this.log("Critical Illness Cover not displayed for TC_15 - stopping journey as pass");
                await this.fullScreenScreenshot("Result Page: Critical Illness Cover Not Displayed Screenshot");
                return true;
            }
        }
    ...
    private async selectCriticalIllnessCover(ciPackage: string, coverAmount?: number) {
        ...
        await this.click(this.getRiderCoverDropdown("Critical Illness Cover"), "click on CI cover dropdown");
    ...
    private async selectAccidentalTotalPermanentDisability(coverAmount?: number) {
        await this.click(this.getRiderCard("Accidental Total & Permanent Disability").locator(".Addons_name__31m6r"), "click on ATPD button");
        await this.click(this.getRiderCoverDropdown("Accidental Total & Permanent Disability"), "click on ATPD cover dropdown");
        await this.submitAddonCover(coverAmount);
    }

    private async selectAccidentalDeathCover(coverAmount?: number) {
        await this.click(this.getRiderCard("Accidental Death Cover").locator(".Addons_name__31m6r"), "click on Accidental Death Cover button");
        await this.click(this.getRiderCoverDropdown("Accidental Death Cover"), "click on ADC cover dropdown");
        await this.submitAddonCover(coverAmount);
    }

    private async selectWaiverOfPremium() {
        await this.click(this.getRiderCheckbox("Waiver of Premium"), "click on Waiver of Premium checkbox");
    }
